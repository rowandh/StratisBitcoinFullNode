...
namespace Stratis.FederatedPeg.IntegrationTests
{
    public class SmartContractTests : IClassFixture<SidechainTestContextFixture>
    {
        ...
        public async Task CanCreateContract()
        {
            ...
            TestHelper.WaitLoop(() => this.context.SideUser.CreateRPCClient().GetRawMempool().Length == 1);
            TestHelper.WaitLoop(() => this.context.FedSide1.CreateRPCClient().GetRawMempool().Length == 1);
            // Block is mined
