...
namespace Stratis.FederatedPeg.IntegrationTests
{
    public class NodeSetupTests
    {
        ...
        [Fact]
        public async Task SideChain_Contract_Execution()
        {
            using (SidechainTestContext context = new SidechainTestContext())
            {
                context.StartAndConnectNodes();

                TestHelper.WaitLoop(() => context.SideUser.FullNode.Chain.Height >= context.SideUser.FullNode.Network.Consensus.PremineHeight + context.SideChainNetwork.Consensus.CoinbaseMaturity);
                TestHelper.WaitForNodeToSync(context.SideUser, context.FedSide1);
                Block block = context.SideUser.FullNode.Chain.GetBlock((int)context.SideChainNetwork.Consensus.PremineHeight).Block;
                Transaction coinbase = block.Transactions[0];
                Assert.Single(coinbase.Outputs);
                ...
                var value = context.GetBalance(context.FedSide1);

                Assert.True(value > 0);

                IList<AddressBalance> balances = context.GetAddressBalances(context.FedSide1);
                string address = balances.First().Address;
                byte[] contractCode = ContractCompiler.CompileFile("SmartContracts/BasicTransfer.cs").Compilation;
                string newContractAddress = await context.SendCreateContractTransaction(context.FedSide1, contractCode, 1, address);

                // A few blocks are mined...
                int currentHeight = context.SideUser.FullNode.Chain.Height;
                TestHelper.WaitLoop(() => context.SideUser.FullNode.Chain.Height >= currentHeight + 3);

                // Contract code is stored as expected
                Assert.NotNull(context.GetContractCode(context.SideUser, newContractAddress));
            }
        }
    }
}
