...
namespace Stratis.Bitcoin.Features.SmartContracts.Tests
{
    public class ExecutorSpecification
    {
        ...
        public void Create_Contract_Success()
        {
            ...
            var rawTransactionData = new byte[] {0xAA, 0xBB, 0xCC};
            var contractTxData = new ContractTxData(1, 1, (Gas) 1000, rawTransactionData);
            ...
            callDataSerializer.Verify(s => s.Deserialize(rawTransactionData), Times.Once);
        ...
        {
            var network = new SmartContractsRegTest();
            uint160 contractAddress = uint160.One;
            var gasConsumed = (Gas)100;
            var rawTransactionData = new byte[] { 0xAA, 0xBB, 0xCC };
            var parameters = new object[] {1, "a"};
            var contractTxData = new ContractTxData(1, 1, (Gas)1000, contractAddress, "Test", "", parameters);
            var refund = new Money(0);
            const ulong mempoolFee = 2UL; // MOQ doesn't like it when you use a type with implicit conversions (Money)
            ISmartContractTransactionContext context = Mock.Of<ISmartContractTransactionContext>(c =>
                c.Data == rawTransactionData &&
            ...
            var stateTransitionResult =
                new StateTransitionResult(gasConsumed, contractAddress, true, vmExecutionResult);

            var internalTransfers = new List<TransferInfo>().AsReadOnly();
            var stateMock = new Mock<IState>();
            stateMock.Setup(s => s.Apply(It.IsAny<ExternalCallMessage>()))
                .Returns(stateTransitionResult);
            stateMock.SetupGet(p => p.InternalTransfers).Returns(internalTransfers);
            ...
            stateMock.Verify(sm => sm
                .Apply(It.IsAny<ExternalCallMessage>()), Times.Once);
